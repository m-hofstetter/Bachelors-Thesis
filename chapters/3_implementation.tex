...
\begin{enumerate}
  ...
  \item \label{item:restrictexpressions} Replace all $B_{Ai}$ with $B_{Ai}\cap(B_{B1}\cup B_{B2}\cup\ldots)$, therefore removing all expressions in the bounds of $A$ and $B$ that are only present in either of them but not both.
  ...
  \item \label{item:traverse} Now traverse the bounds of both FunArrays from left to right.
  \begin{enumerate}[label*=\arabic*.]
  	\item If $B_{Ai} = B_{Bi}$, go on go on to the next bounds $B_{Ai+1}$ and $B_{Bi+1}$.
    ...
    \item \label{item:truesubset} If $B_{Bi} \subsetneqq B_{Ai}$, add a neutral segment, so that $A=\ldots B_{Ai}P_{Ai}\allowbreak B_{Ai+1}\ldots$ becomes $A'=\ldots (B_{Ai} \cup B_{Bi}) \;\nElement_A\, (B_{Ai} \setminus B_{Bi}) P_{Ai}B_{Ai+1}\ldots$
    \item If $B_{Ai} \subsetneqq B_{Bi}$, do the analogous for $B$.
  \end{enumerate}
