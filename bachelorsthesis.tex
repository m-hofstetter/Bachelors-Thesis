...
Table \ref{table:intervalarithmetics} shows the supported expression arithmetics and how they are implemented. 

The modulo operation over-approximates the result

\include{res/interval_arithmetics}
...
\begin{tabular}{l|l}
           ...
           meet & $[\underline{x},\overline{x}]\sqcap[\underline{y},\overline{y}]=[\max(\underline{x},\underline{y}),\min(\overline{x}, \overline{y})]$ or $\bot$, if $\max(\underline{x},\underline{y})>\min(\overline{x}, \overline{y})$\\
           widening&fhg
           
           
        \end{tabular}
  \caption{Supported abstract operations for the  implemented interval domain. \cite{cousot1992}} \label{table:intervalabstractoperations}
  \end{center}
\end{table}
\endgroup
