...
\section{Implementing the FunArray}
\subsection{Unifying two FunArrays}
\section{Implementing the Analyser}
...
\section{Limitations}
\subsection{Relations between arrays}

A lot of the benchmarks follow a scheme where they first initialise an array with certain values and then copy each element sequentially to another array. It then tries to assert whether the two arrays are equal or fulfil another specified relation. However in present configuration of the FunArray, this is not possible, \textcolor{red} {TODO} %todo: continue thsi
